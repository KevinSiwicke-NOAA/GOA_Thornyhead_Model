...
# Model 22 = m22: accepted in 2022, 3 process errors, 1 q, xtra observation error for BTS and LLS
# Model 24 = m24: 1 process error 1 q, xtra observation error for BTS and LLS
...
# Model 22_3_PE -----
input <- prepare_rema_input(model_name = 'Model 22_3pe_est_Xtra',
                            multi_survey = 1,
...
# Model 24.1_3_PE -----
# Same as M22, but single PE
input <- prepare_rema_input(model_name = 'Model 24.1_1pe_est_Xtra',
                            multi_survey = 1,
                            biomass_dat = biomass_dat,
                            cpue_dat = cpue_dat,
                            sum_cpue_index = TRUE,
...
compare <- compare_rema_models(rema_models = list(m22, m24.1))
...
ggsave(filename = paste0(out_path, '/M22_3pe_vs_M24.1_1pe_fits.png'),
...
ggsave(filename = paste0(out_path, '/M22_3pe_vs_M24.1_1pe_totalbiomass.png'),
...
old <- out22$total_predicted_biomass %>% filter(year == 2024) %>% pull(pred)
new <- out24$total_predicted_biomass %>% filter(year == 2024) %>% pull(pred)
(new-old)/old
